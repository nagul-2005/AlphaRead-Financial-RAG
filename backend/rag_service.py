...
class RAGEngine:
    ...
    def chat(self, query: str) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            "2. NO RAW MARKDOWN TABLES: Do NOT output raw Markdown grid tables (do NOT use '| Header | Header |' or '| :--- |' table syntax). Convert all numbers and financial data into clean narrative text and bullet points and don't use ** symbol for answer text.\n"
        ...
        if groq_key:
            answer, groq_err_msg = self._call_groq_api(system_prompt, user_prompt, groq_key)

        if not answer:
            answer = self._generate_fallback_answer(query, relevant_chunks, groq_err_msg)
    ...
    def _call_groq_api(self, system_prompt: str, user_prompt: str, api_key: str) -> tuple[str, str]:
        ...
        if not api_key:
            return "", "Groq API key is missing from environment variables."
